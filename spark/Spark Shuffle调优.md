...
* 默认值：3(可以调大，多重试几次，避免因为gc，网络延迟而连接不上)
...
* 默认值：200，参数的意义是是否开启bypass的阈值
* 参数说明：当ShuffleManager为SortShuffleManager时，如果shuffle read task的数量小于这个阈值（默认是200），则shuffle write过程中不会进行排序操作(**没有排序才能使用bypass操作**)，而是直接按照未经优化的HashShuffleManager的方式去写数据，但是最后会将每个task产生的所有临时磁盘文件都合并成一个文件，并会创建单独的索引文件。
